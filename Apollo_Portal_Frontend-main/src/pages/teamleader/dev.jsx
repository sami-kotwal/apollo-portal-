...
function DevLeaderHome() {
  ...
  useEffect(() => {
    ...
    Promise.all([API.get("/tasks"), API.get("/users")]).then(([taskRes, userRes]) => {
      ...
      setUsers(userRes.data.filter((member) => member.role === "developer" || member.role === "teamleader_dev"));
    });
  ...
  return (
    <DashboardAnalytics
      title={`Welcome ${user?.name || "Team Leader"}`}
      subtitle="Track development progress, team members, task timing, deadlines, and recent work history."
      tasks={tasks}
      users={users}
      showPeople
...
function AllTasks() {
  ...
  return (
    <>
      <h2 className="text-xl font-bold mb-4">All Development Tasks</h2>
...
function AssignTasks() {
  ...
  return (
    <>
      ...
      <div className="mb-6 rounded-xl border border-slate-800 bg-slate-900/80 p-5 shadow-lg shadow-slate-950/20">
        ...
        <p className="mt-1 text-sm text-slate-400">How many times each developer could not continue and the task was taken back.</p>
  ...
  <TaskCard key={task._id} task={task}>
    ...
    {task.status !== "completed" && (
      ...
      >
        <option value="" className="bg-white text-black">
          Select developer or assign to yourself
        </option>
    ...
    {/* 🔁 TAKE BACK TASK */}
    {task.status !== "completed" && getId(task.assignedTo) && getId(task.assignedTo) !== user?._id && (
      <button
